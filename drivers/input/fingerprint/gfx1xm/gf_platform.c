...
void gf_cleanup(struct gf_dev* gf_dev)
{
	...
	if (gpio_is_valid(gf_dev->irq_gpio))
	{
		pinctrl_free_gpio(gf_dev->irq_gpio);
	...
	if (gpio_is_valid(gf_dev->pw_gpio))
	{
		gpio_free(gf_dev->pw_gpio);
		pr_info("remove pw_gpio success\n");
	}
	if (gpio_is_valid(gf_dev->miso_gpio))
	{
		gpio_free(gf_dev->miso_gpio);
		pr_info("remove miso_gpio success\n");
	}
}
