...
#define IOCTL_GET_IMAGE_KERNEL _IOW(ELAN_IOCTLID, 14, int)
...
static void elan_fp_gpio_int_config(struct elan_fp_data *fp)
{
	...
	err = request_threaded_irq(fp->elan_irq, NULL, elan_fp_irq_handler,\
		IRQF_TRIGGER_RISING | IRQF_ONESHOT, ELAN_FP_NAME, fp);
	if (err != 0){
...
static int elan_fp_alloc_image_buffer(struct elan_fp_data *fp)
{
	...
	alloc_len = ((max_width_pix*2+1)*max_heigh_pix + 1 + (SPI_ALINE_BYTE-1))/ SPI_ALINE_BYTE * SPI_ALINE_BYTE;
